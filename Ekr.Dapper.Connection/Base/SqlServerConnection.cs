...
namespace Ekr.Dapper.Connection.Base
{
    public class SqlServerConnection : IBaseConnection
    {
        ...
        public T WithConnection<T>(Func<IDbConnection, T> func)
        {
            try
            {
                //24-01-24 sementara ditutup untuk kebutuhan pembacaan connection string melalui environment variable windows
                using var db = new OrmLiteConnectionFactory
                    (_connectionString, SqlServerDialect.Provider).OpenDbConnection();

                //24-01-24 environment variables (start)
                //var env = Environment.GetEnvironmentVariable(_connectionString);
                //using var db = new OrmLiteConnectionFactory
                //    (env, SqlServerDialect.Provider).OpenDbConnection();
                //(end)
